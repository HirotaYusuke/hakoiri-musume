...
- 追加パックは2種で、購入すると問題選択画面に現れます。
  - `rush-pack-1`（EX1-EX12）: 本編にない10駒構成、最短15〜21手。
  - `rush-pack-2`（EXX1-EXX12）: 12駒の高密度構成、最短26〜37手・操作15〜19手の最上位帯。
- 難問36問とパック24問の固定データは `npx tsx src/scripts/regenerate-rush-specs.ts` で一括再生成します。解法の同一・類似（編集距離）、初期配置の1駒違いを排除して選定します。再生成後は `--print-hard-test-metrics` で固定メトリクスを更新してください。
